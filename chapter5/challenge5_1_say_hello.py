...
class SayHello(Node):
  """
  Create a SayHello class, which is a subclass of the Node class.
  """
  def __init__(self):
    """
    ...
    super().__init__('challenge5_1_say_hello')
...
def main(args=None):
  ...
  # Create the node
  challenge5_1_say_hello = SayHello()
  
  # Spin the node so the callback function is called.
  rclpy.spin(challenge5_1_say_hello)
  
  # Destroy the node explicitly
  # (optional - otherwise it will be done automatically
  # when the garbage collector destroys the node object)
  challenge5_1_say_hello.destroy_node()
